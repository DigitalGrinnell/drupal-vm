...
# Define the root name of the backup
backup = "rs.tar.gz"
cwd = os.getcwd()
stick = "/Volumes/ROOTSTALK"
...
# Define the drush art command and run it remotely via ssh.  check_call returns 0 if no error, or a traceback if there is
...
error = subprocess.check_call([ "ssh", "dguser@dgdevx.grinnell.edu", drush_ard ]);

# No problem...rsync the file back to the host
args = [ "rsync", "-aruvi", "dguser@dgdevx.grinnell.edu:" + destination, cwd ]
print style.BOLD + "\nRunning '" + ' '.join(args) + "' to copy the backup to your host..."  + style.END
error = subprocess.check_call(args)

# If /Voliumes/ROOTSTALK is mounted, copy the backup there too
if os.path.isdir(stick):
    args = ["rsync", "-aruvi", local, stick]
    print style.BOLD + "\nRunning '" + ' '.join(args) + "' to copy the backup to your mounted " + stick + " valume..."  + style.END
    error = subprocess.check_call(args)
